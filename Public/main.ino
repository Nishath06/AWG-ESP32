...
// ================= NEW ADDITIONS =================
#define ONE_WIRE_BUS 2     // DS18B20 (changed to avoid conflict)
...
void checkResetButton() {
  int state = digitalRead(RESET_BUTTON);
  ...
  if (state == LOW) {
    ...
    if (millis() - pressStart > 2000) {
      Serial.println("FORCE RESET WiFi...");
...
void connectAWS() {
  ...
  while (!client.connected() && millis() - startAttempt < 3000) {

    checkResetButton();  // already good ✅
...
void loop() {
  ...
  controlHeater();   // ✅ added safely (non-blocking)
